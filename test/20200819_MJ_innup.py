...
for inf in info:
    ...
    if len(res) > 0:
    # update
        cur.execute(update_query, [int(inf['naver_idx']), inf['r_name'], inf['r_category'],inf['price'], inf['image_url'], inf['distance'], int(inf['site_score']), int(inf['site_review']), inf['main_menu']])
    else:
    # insert
